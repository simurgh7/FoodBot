...
def main():
    ...
    try:
        os.remove(training_args.output_dir+'/log.log')
    except FileNotFoundError:
        print('not found')
    logging.basicConfig(
        filename=training_args.output_dir+'/log.log',
    ...
    def convert_label_to_list(example):
        # Assuming 'Label' is the column containing string representations of lists
        example[data_args.label_column_name] = eval(example[data_args.label_column_name])
        return example
    ...
    if data_args.dataset_name is not None:
        ...
        if data_args.is_multi_label:
            raw_datasets = raw_datasets.map(convert_label_to_list)
    ...
    if is_regression:
        ...
        if raw_datasets["train"].features["label"].dtype == "list":  # multi-label classification
            print('is_regression Label type is list')
            is_multi_label = True
        ...
        if trainer.is_world_process_zero():
            with open(output_predict_file, "w") as writer:
                ...
                for index, item in enumerate(predictions):
                    if is_regression:
                        writer.write(f"{index}\t{item:3.3f}\t{labels[index]:3.3f}\n")
                    elif is_multi_label:
                        # recover from multi-hot encoding
                        item = [label_list[i] for i in range(len(item)) if item[i] == 1]
                        label = [label_list[i] for i in range(len(labels[index])) if labels[index][i] == 1]
                        writer.write(f"{index}\t{item}\t{label}\n")
                    else:
                        item = label_list[item]
                        label = label_list[labels[index]]
                        writer.write(f"{index}\t{item}\t{label}\n")
